...
*business-unit-internal*::
  ...
  specific product portfolio owned by the same business unit (e.g IST Products teams).
*company-internal*::
  The API consumers with this audience are restricted to applications owned
  by the business units of the same the company (e.g. different teams within the IST group)
*external-partner*::
...
[#224]
== {MUST} follow naming convention for basepaths

Basepaths in APIs must, respectively should conform to the functional naming
depending on the <<219, audience>> as follows (see <<223>> for details and
`<functional-name>` definition):

[source,bnf]
-----
<basepath>             ::= <functional-hostname> | <application-hostname>

<functional-hostname>  ::= api.ist.com/<functional-name>
-----
...
[source,bnf]
-----
<application-hostname> ::= <application-id>.<organization-unit>.ist.com
